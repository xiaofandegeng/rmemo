...
- rehearsal guardrails: `--archive-verify` requires `--archive`; `--archive-require-files`/`--archive-require-preset` require `--archive-verify`; `--archive-snapshot-id`/`--snapshot-id`/`--archive-retention-days`/`--archive-max-snapshots-per-version` require `--archive`
...
- `release-archive-find` guardrails: `--snapshot-id`, `--require-files`, and `--require-preset` require `--version`
- `release-archive-find` guardrails: `--list-require-presets` cannot be combined with `--version/--snapshot-id/--require-files/--require-preset`
- `release-archive-find` JSON includes integration-friendly summary block (`standardized.status/resultCode/checkStatuses/failureCodes/failures`), including `--list-require-presets` mode
...
- in `--archive` mode, `artifacts/release-summary.json` is still written as a compatibility machine-readable summary even when the primary summary output is markdown
- `release-archive` now snapshots `release-summary.md` as well (when present), not only `release-summary.json`
