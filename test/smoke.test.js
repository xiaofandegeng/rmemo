...
test("rmemo ws focus snapshots can be saved, listed and compared", async () => {
  ...
  const alertsCheck = await runNode([rmemoBin, "--root", tmp, "--format", "json", "ws", "alerts", "check"]);
  assert.equal(alertsCheck.code, 0, alertsCheck.err || alertsCheck.out);
  const alertsCheckJson = JSON.parse(alertsCheck.out);
  assert.equal(alertsCheckJson.ok, true);
  assert.ok(alertsCheckJson.incident && alertsCheckJson.incident.id);

  const alertsHistory = await runNode([rmemoBin, "--root", tmp, "--format", "json", "ws", "alerts", "history", "--limit", "10"]);
  assert.equal(alertsHistory.code, 0, alertsHistory.err || alertsHistory.out);
  const alertsHistoryJson = JSON.parse(alertsHistory.out);
  ...
  const alertsRca = await runNode([rmemoBin, "--root", tmp, "--format", "json", "ws", "alerts", "rca", "--incident", alertsCheckJson.incident.id]);
  assert.equal(alertsRca.code, 0, alertsRca.err || alertsRca.out);
  const alertsRcaJson = JSON.parse(alertsRca.out);
...
test("rmemo mcp serves tools over stdio (status + search)", async () => {
  ...
  mcp.writeLine({
    ...
    id: 10,
    method: "tools/call",
    params: { name: "rmemo_memory_search", arguments: { root: tmp, format: "json", q: "Rules", limit: 10 } }
  });

  await waitFor(() => {
    const lines = parseJsonLines(mcp.getOut());
    const ids = new Set(lines.map((x) => x.id));
    return [3, 4, 5, 6, 7, 8, 9, 10].every((id) => ids.has(id));
  });
  ...
  assert.ok(list.result.tools.some((t2) => t2.name === "rmemo_memory_search"));
  ...
  const memory = lines.find((x) => x.id === 10);
  const memoryJson = JSON.parse(memory.result.content[0].text);
  assert.equal(memoryJson.schema, 1);
  assert.ok(Array.isArray(memoryJson.entries));
...
test("rmemo mcp workspace tools list and focus across subprojects", async () => {
  ...
  try {
    ...
    mcp.writeLine({
      ...
      id: 15,
      method: "tools/call",
      params: { name: "rmemo_ws_focus_alerts_history", arguments: { root: tmp, limit: 10 } }
    });
    await waitFor(() => {
      const lines11 = parseJsonLines(mcp.getOut());
      return lines11.some((x) => x.id === 15);
    });
    const lines11 = parseJsonLines(mcp.getOut());
    const wsAlertsHistory = lines11.find((x) => x.id === 15);
    const wsAlertsHistoryJson = JSON.parse(wsAlertsHistory.result.content[0].text);
    assert.equal(wsAlertsHistoryJson.schema, 1);
    assert.ok(Array.isArray(wsAlertsHistoryJson.incidents));
  } finally {
    mcp.closeIn();
    try {
      mcp.p.kill("SIGTERM");
...
test("rmemo mcp --allow-write exposes write tools and can update repo memory", async () => {
  ...
  try {
    ...
    mcp.writeLine({
      ...
      method: "tools/call",
      params: {
        name: "rmemo_ws_focus_alerts_check",
        arguments: { root: tmp, limitGroups: 10, limitReports: 100, autoGovernance: false, source: "test-mcp-write" }
      }
    });
    mcp.writeLine({
      jsonrpc: "2.0",
      ...
      method: "tools/call",
      params: {
        name: "rmemo_ws_focus_alerts_history",
    ...
    mcp.writeLine({
      ...
      method: "tools/call",
      params: {
        name: "rmemo_ws_focus_alerts_rca",
    ...
    mcp.writeLine({
      ...
      method: "tools/call",
      params: {
        name: "rmemo_memory_extract",
        arguments: { root: tmp, recentDays: 7, limit: 200, source: "test-mcp" }
      }
    });
    mcp.writeLine({
      jsonrpc: "2.0",
      ...
      method: "tools/call",
      params: {
        name: "rmemo_memory_write",
        arguments: {
          root: tmp,
          key: "manual:test-mcp-memory",
          title: "MCP memory entry",
          summary: "Track MCP write path coverage in smoke test.",
          tags: ["mcp", "memory"],
          modules: ["src/core/mcp.js"],
          source: "test-mcp"
        }
      }
    });
    mcp.writeLine({
      ...
      method: "tools/call",
      params: {
        name: "rmemo_memory_search",
        arguments: { root: tmp, format: "json", q: "MCP memory entry", limit: 10 }
      }
    });
    mcp.writeLine({
      jsonrpc: "2.0",
    ...
    await waitFor(() => {
      const lines2 = parseJsonLines(mcp.getOut());
      return lines2.some((x) => x.id === 22) && lines2.some((x) => x.id === 23) && lines2.some((x) => x.id === 36);
    });
    ...
    assert.ok(list.result.tools.some((t2) => t2.name === "rmemo_memory_search"));
    assert.ok(list.result.tools.some((t2) => t2.name === "rmemo_memory_extract"));
    assert.ok(list.result.tools.some((t2) => t2.name === "rmemo_memory_write"));
    assert.ok(list.result.tools.some((t2) => t2.name === "rmemo_memory_link"));
    ...
    const memExtract = lines.find((x) => x.id === 33);
    const memExtractJson = JSON.parse(memExtract.result.content[0].text);
    assert.equal(memExtractJson.schema, 1);
    assert.ok(typeof memExtractJson.totalEntries === "number");

    const memWriteCreate = lines.find((x) => x.id === 34);
    const memWriteCreateJson = JSON.parse(memWriteCreate.result.content[0].text);
    assert.equal(memWriteCreateJson.schema, 1);
    assert.equal(typeof memWriteCreateJson.created, "boolean");
    assert.ok(memWriteCreateJson.entry.id);
    assert.equal(memWriteCreateJson.entry.key, "manual:test-mcp-memory");

    const memSearch = lines.find((x) => x.id === 35);
    const memSearchJson = JSON.parse(memSearch.result.content[0].text);
    assert.equal(memSearchJson.schema, 1);
    assert.ok(Array.isArray(memSearchJson.entries));

    const memWriteUpdate = lines.find((x) => x.id === 36);
    const memWriteUpdateJson = JSON.parse(memWriteUpdate.result.content[0].text);
    assert.equal(memWriteUpdateJson.schema, 1);
    assert.equal(typeof memWriteUpdateJson.created, "boolean");
    assert.equal(memWriteUpdateJson.entry.status, "wip");
    ...
    const wsAlertsCheck = lines.find((x) => x.id === 17);
    const wsAlertsCheckJson = JSON.parse(wsAlertsCheck.result.content[0].text);
    assert.equal(wsAlertsCheckJson.ok, true);
    assert.ok(wsAlertsCheckJson.incident && wsAlertsCheckJson.incident.id);

    const wsAlertsHistory = lines.find((x) => x.id === 18);
    const wsAlertsHistoryJson = JSON.parse(wsAlertsHistory.result.content[0].text);
    assert.equal(wsAlertsHistoryJson.schema, 1);
    assert.ok(Array.isArray(wsAlertsHistoryJson.incidents));

    const wsAlertsRca = lines.find((x) => x.id === 19);
    if (wsAlertsRca.error) {
      assert.ok(String(wsAlertsRca.error.message || "").length > 0);
    } else {
