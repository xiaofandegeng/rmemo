...
test("release-rehearsal summary aggregates standardized failure codes from failed step json output", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "rmemo-release-rehearsal-step-codes-"));
  await fs.mkdir(path.join(tmp, "scripts"), { recursive: true });
  ...
  await fs.writeFile(
    ...
    [
      ...
      "      failures: [",
      "        { check: 'tests', code: 'READY_CHECK_TESTS_FAILED', retryable: false }",
      "      ]",
      "    }",
      "  }, null, 2) + '\\n');",
      "  process.exit(1);",
  ...
  assert.equal(r.code, 1, r.err || r.out);
  const report = JSON.parse(r.out);
  assert.equal(report.standardized.failureCodes.includes("READY_CHECK_TESTS_FAILED"), true);
  assert.equal(
    report.standardized.failures.some((x) => x.step === "release-ready-json" && x.code === "READY_CHECK_TESTS_FAILED"),
    true
  );

  const summary = JSON.parse(await fs.readFile(path.join(tmp, summaryPath), "utf8"));
  const readyJsonFailure = summary.failedSteps.find((x) => x.name === "release-ready-json");
  assert.ok(readyJsonFailure);
  assert.equal(readyJsonFailure.downstreamFailureCodes.includes("READY_CHECK_TESTS_FAILED"), true);
  assert.equal(summary.summaryFailureCodes.includes("READY_CHECK_TESTS_FAILED"), true);
...
test("release-rehearsal supports --version current alias from package.json", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "rmemo-release-rehearsal-version-current-"));
  await fs.mkdir(path.join(tmp, "scripts"), { recursive: true });

  await fs.writeFile(
    path.join(tmp, "package.json"),
    JSON.stringify({ name: "test-release-rehearsal", version: "9.9.8", type: "module" }, null, 2) + "\n",
    "utf8"
  );
  await fs.writeFile(path.join(tmp, "scripts", "release-notes.js"), "process.stdout.write('# notes\\n');\n", "utf8");
  await fs.writeFile(path.join(tmp, "scripts", "release-ready.js"), "process.stdout.write('# ready\\n');\n", "utf8");
  ...
  assert.equal(r.code, 0, r.err || r.out);
  const report = JSON.parse(r.out);
  assert.equal(report.version, "9.9.8");
});

test("release-rehearsal rejects --version current when package version is missing", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "rmemo-release-rehearsal-version-current-missing-"));
  await fs.mkdir(path.join(tmp, "scripts"), { recursive: true });

  await fs.writeFile(
    path.join(tmp, "package.json"),
    JSON.stringify({ name: "test-release-rehearsal", type: "module" }, null, 2) + "\n",
    "utf8"
  );
  await fs.writeFile(path.join(tmp, "scripts", "release-notes.js"), "process.stdout.write('# notes\\n');\n", "utf8");
  await fs.writeFile(path.join(tmp, "scripts", "release-ready.js"), "process.stdout.write('# ready\\n');\n", "utf8");
  ...
  const r = await runNode(
    [path.resolve("scripts/release-rehearsal.js"), "--root", tmp, "--version", "current", "--repo", "owner/repo"],
    { cwd: path.resolve("."), env: { ...process.env } }
  );
...
test("release-rehearsal bundle enables archive + archive-verify flow", async () => {
  ...
  const summary = JSON.parse(await fs.readFile(path.join(tmp, "artifacts", "release-summary.json"), "utf8"));
  assert.equal(summary.bundleResolved, "rehearsal-archive-verify");
  assert.equal(summary.archive.verify.requiredFilesPreset, "rehearsal-archive-verify");
});

test("release-rehearsal bundle writes bundleResolved in markdown summary", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "rmemo-release-rehearsal-bundle-summary-md-"));
  await fs.mkdir(path.join(tmp, "scripts"), { recursive: true });
  ...
  await fs.writeFile(
    path.join(tmp, "scripts", "release-archive.js"),
    [
      "process.stdout.write(JSON.stringify({ ok: true, snapshotId: '20260226_161500' }, null, 2) + '\\n');",
      "process.exit(0);"
    ].join("\n"),
    "utf8"
  );
  ...
  assert.match(summaryMd, /bundleResolved: rehearsal-archive-verify/);
  const summaryJsonCompat = JSON.parse(await fs.readFile(path.join(tmp, "artifacts", "release-summary.json"), "utf8"));
  assert.equal(summaryJsonCompat.bundleResolved, "rehearsal-archive-verify");
});
...
test("release-rehearsal archive mode writes markdown summary by default when summary-format=md", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "rmemo-release-rehearsal-archive-summary-md-"));
  await fs.mkdir(path.join(tmp, "scripts"), { recursive: true });
  ...
  await fs.writeFile(
    path.join(tmp, "scripts", "release-archive.js"),
    [
      "process.stdout.write(JSON.stringify({ ok: true, snapshotId: '20260225_132000' }, null, 2) + '\\n');",
      "process.exit(0);"
    ].join("\n"),
    "utf8"
  );
