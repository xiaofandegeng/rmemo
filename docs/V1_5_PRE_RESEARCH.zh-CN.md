...
更新时间：2026-02-27  
状态：v1.5 M1-M5 已完成（已收口，执行入口切换到 v1.6）
...
- [x] 新增 `--bundle` 显式执行摘要字段（例如 `bundleResolved`）到 `release-summary.json/.md`，降低排障时对参数回放依赖。
...
- `v1.5` 的 M1-M5 任务均已完成。
- 后续迭代默认从 `docs/NEXT_CYCLE_PLAN.md`（v1.6）和 `docs/V1_6_CYCLE_PLAN.zh-CN.md` 进入。
