...
### 里程碑 C（`v1.4.0`）：发布与文档闭环（已完成）

- [x] 完成 `v1.4.0` 的 npm 与 GitHub Release 双端可见性验证（见 `docs/RELEASE_VERIFICATION_v1.4.0.md`）。
- [x] 更新 runbook（异常排障路径、严格模式说明、资产命名约束）。
- [x] 校准 `docs/TASKS.md` 与下一周期入口（切换到 v1.5 预研）。

验收标准：

- [x] 发布后 30 分钟内可完成端到端核验并留存审计文件。
- [x] 新接力模型仅阅读当前文档即可独立完成一次发布演练。
